...
-- Q05) 위에서 작성한 쿼리문을 복사해 붙여 넣은 후 전체금액이 8,500,000 이상인 '서울' 지역 고객만 조회되게 수정하세요.
 SELECT o.order_id, o.customer_id, c.customer_name, o.employee_id, e.employee_name, o.order_date, o.total_due
    ...
    JOIN customer AS c ON o.customer_id = c.customer_id
    JOIN employee AS e ON o.employee_id = e.employee_id
    WHERE o.total_due >= 8500000 AND c.city = '서울' ;
...
SELECT p.product_id, p.product_name, p.sub_category_id, s.category_id
	FROM product AS p
    JOIN sub_category AS s ON p.sub_category_id = s.sub_category_id;


-- Q11) 위에서 작성한 쿼리문을 복사해서 붙여 넣은 후 대분류이름이 조회되게 수정하세요.
SELECT p.product_id, p.product_name, p.sub_category_id, s.category_id, c.category_name
	FROM product AS p
    JOIN sub_category AS s ON p.sub_category_id = s.sub_category_id
...
SELECT c.category_name, s.sub_category_name, p.product_name, p.product_id
	FROM product AS p
    JOIN sub_category AS s ON p.sub_category_id = s.sub_category_id
    JOIN category AS c ON s.category_id = c.category_id;
