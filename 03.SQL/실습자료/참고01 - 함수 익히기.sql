...
-- 6) INSTR() - 문자열에서 지정한 문자열이 나타나는 위치 확인, 없으면 0
...
-- 10) UPPER() - 모든 문자를 대문자로 변경

SELECT UPPER('I have a Dream.'); -- I HAVE A DREAM.


-- 11) LOWER() - 모든 문자를 소문자로 변경
...
-- 16) TRIM() - 양쪽 공백, 또는 지정한 문자 제거

SELECT CONCAT('[', TRIM('   대한민국   '), ']'); -- [대한민국]


-- 17) REPLACE() -- 문자열의 특정 문자열 변경

SELECT REPLACE('우리나라 대한민국', ' ', ''); -- 우리나라대한민국


-- 18) REPEAT() -- 문자열을 지정한 룃수만큼 반복
...
-- 21) SUBSTRING() -- 지정한 위치에서 지정한 길이 만큼의 문자열 추출
...
-- 34) DATE() -- 지정한 날짜와 시간에서 날짜 확인

SELECT DATE(NOW()); -- 2020-01-06


-- 35) TIME() -- 지정한 날짜와 시간에서 시간 확인
...
-- 48) GREATEST() - 나열된 값 중에서 가장 큰 값 확인

SELECT GREATEST(10, 12, 11, 34, 21); -- 34


-- 49) LEAST() - 나열된 값 중에서 가장 작은 값 확인
...
-- 50) IFNULL() - NULL 값은 다른 값으로 바꾸기

SELECT IFNULL(NULL, 10); -- 10


-- 51) NULLIF() - 두 값이 같으면 NULL

SELECT NULLIF(10, 10); -- NULL


-- 52) COALESCE() - 나열된 값 중에서 NULL이 아닌 첫번 째 값 확인
