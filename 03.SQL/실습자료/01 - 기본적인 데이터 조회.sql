...
SELECT *
	FROM employee
    WHERE hire_date >= '20180101' AND dept_id = 'SYS' ;


-- Q) SYS 부서 직원 중에서 급여를 6,000 이상 받는 직원 정보 조회
SELECT *
	FROM employee
	WHERE salary >= 6000 AND dept_id = 'SYS' ;
...
SELECT *
	FROM vacation
    WHERE emp_id IN ('S0001', 'S0003', 'S0005') AND begin_date BETWEEN '20150101' AND '20151231' ;
...
SELECT CONCAT(emp_id, '(', dept_id, ')') AS emp_id,
	emp_name, eng_name, gender, hire_date, retire_date, phone, email, salary
	FROM employee 
    WHERE retire_date IS NULL;
