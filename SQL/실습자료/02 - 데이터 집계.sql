...
select *
	from vacation 
    where emp_id = 's0001' and begin_date between '20140101' and '20141231';
...
SELECT emp_id, sum(duration) as tot_duration
	FROM vacation
    where begin_date between '20170101' and '20171231'
    GROUP BY emp_id
...
SELECT emp_id, COUNT(*) AS num_duration
	FROM vacation 
    WHERE begin_date BETWEEN '20170101' AND '20171231'
    GROUP BY emp_id
    HAVING COUNT(*) >= 3;
...
-- 리뷰
-- 남자직원 수 집계
SELECT COUNT(*) AS count_man, 
		SUM(COALESCE(salary, 0)) AS tot_salary, 
		ROUND(AVG(COALESCE(salary, 0))) AS avg_salary
	FROM employee
    WHERE gender='M' ;
...
SELECT dept_id, 
		SUM(CASE WHEN gender = 'M' THEN salary
			ELSE 0 END) AS M_salary,
		SUM(CASE WHEN gender = 'F' THEN salary
			ELSE 0 END) AS F_salary,
